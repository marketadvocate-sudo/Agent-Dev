...
in `MWR_CONSTITUTION.md` v1.4 Section 7. The reviewer rules against the constitution,
...
- `mwr_version` graded against: `______` (must be `1.4`)
...
## 3. Standards check (constitution Section 4) — per shipped exemplar

For each exemplar, mark every standard MET / MISS. No partial credit. Then count
misses and grade.

| # | Standard | MET / MISS | Basis or reason |
|---|----------|------------|-----------------|
| 1 | Independently useful (delivers value with no ask) | | |
| 2 | Relates to the value prop | | |
| 3 | Based on public data (public-classed source only) | | |
| 4 | Translates to meaningful insight (not raw data) | | |
| 5 | Goes beyond pain identification | | |
| 6 | Creates information asymmetry (public sources only) | | |
| 7 | Concrete and specific (type-spanning, see below) | | |
...
- 2+ misses → **PQS**, rejected; the agent declines and names the gap.
